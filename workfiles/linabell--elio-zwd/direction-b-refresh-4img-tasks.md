...
`R0` 至 `R3` 已完成。`R4` 尚未完成：新基线的 identity rows 仍需统一为透明生产行，其中 review 必须无道具重做。`R5` 已完成一次本地 running-right 候选生成与 QA，但候选 01 未通过两相交替步态验收；下一次将改用视觉化步态锚点，尚无可用生产素材。`R6`、`R7` 未开始。
