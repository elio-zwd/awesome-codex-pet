...
`R0` 至 `R3` 已完成。`R4` 尚未完成：新基线的 identity rows 仍需统一为透明生产行，其中 review 必须无道具重做。`R5` 已完成 running-right：用户批准候选 03，8 帧已透明化、注册为 `192 × 208` 并组装为 `1536 × 208` 行，行级检查零错误、零警告；其余 standard rows 尚未开始。`R6`、`R7` 未开始。
